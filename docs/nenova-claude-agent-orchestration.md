...
This orchestration must follow the shared algorithm-development contract in
`docs/nenova-agent-algorithm-development.md`. The agents are not only answering
questions; they must improve the next data fusion, validation, assistance, and
...
- Algorithm learning note: what should be improved in the next collection,
  fusion, UI, endpoint, or assistance loop.
...
- Codex-Claude validation packets are generated by
  `node scripts/nenova-cross-agent-validation.js`, and Claude review can be
  requested with `node scripts/nenova-cross-agent-validation.js --claude`.
