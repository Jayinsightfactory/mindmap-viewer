...
- 모든 수집/병합/검증 결과는 `docs/nenova-agent-algorithm-development.md`
  의 알고리즘 디벨롭 계약을 따라야 합니다. `nenova.exe` 이벤트는 단순
  로그가 아니라 다음 Work Unit 판정, 직원 보조, 관리자 요약, 자동화 후보,
  Codex-Claude 개발 루프의 학습 근거입니다.
