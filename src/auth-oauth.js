...
function createOAuthRouter({ passport, enabledProviders, insertToken, CLIENT_ORIGIN }) {
  ...
  if (enabledProviders.includes('google')) {
    ...
    router.get('/google', (req, res, next) => {
      // mywork: 팝업 로그인 → /oauth-done.html 이 opener(my-work·work-unified)에 토큰을 postMessage 로 넘긴다.
      const nextTarget = req.query.next === 'app' ? 'app' : req.query.next === 'mywork' ? 'mywork' : 'orbit3d';
    ...
    router.get('/google/callback',
      ...
      async (req, res) => {
        ...
        if (req.query.state === 'mywork') return res.redirect(`${origin}/oauth-done.html?oauth_token=${token}&provider=google`);
