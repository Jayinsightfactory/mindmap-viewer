...
try {
  ...
  db.pragma('busy_timeout = 5000');   // 동시 접근 시 5초 대기 (15유저 대응)
  db.pragma('synchronous = NORMAL');  // 쓰기 성능 개선 (WAL 안전)
} catch (e) {
...
// db-pg.js pool을 lazy하게 가져옴 (모듈 로드 시점 순환참조 방지)
let _pgPool = null;
function _initPgPool() {
  if (_pgPool) return;
  // 독립 pool 생성 (db-pg.js require는 lazy하게 _pgInit에서만)
