...
// heartbeat 진단용 상태
let _kbLastFlushAt   = 0;  // 마지막 성공 keyboard.chunk flush
let _kbLastErrorAt   = 0;
let _kbErrorCount    = 0;
let _kbLastErrorMsg  = '';
let _kbFlushCount    = 0;
let _uiohook         = null;
...
function analyzeAndSummarize(rawBuffer, context = {}) {
  ...
  return {
    ...
    // 원본 키스트로크 내용 없음 (실제 전송은 _emit 페이로드의 inputText에서 처리 — 옵션2)
  };
}
    ...
    const req = mod.request({
      ...
      res.on('end', () => {
        if (res.statusCode < 300) {
          ...
          _kbErrorCount++;
          _kbLastErrorAt = Date.now();
          _kbLastErrorMsg = `HTTP ${res.statusCode}`;
...
function getStatus() {
  const now = Date.now();
  const sinceFlush = _kbLastFlushAt ? Math.round((now - _kbLastFlushAt) / 1000) : null;
  let state = 'ok';
  ...
  return {
    ...
    paused:       _paused,
    state,
    flushCount:   _kbFlushCount,
    lastFlushAt:  _kbLastFlushAt ? new Date(_kbLastFlushAt).toISOString() : null,
    secondsSinceFlush: sinceFlush,
    errorCount:   _kbErrorCount,
    lastErrorAt:  _kbLastErrorAt ? new Date(_kbLastErrorAt).toISOString() : null,
    lastErrorMsg: _kbLastErrorMsg || null,
    queueSize:    _remoteBatchQueue.length,
  };
}
