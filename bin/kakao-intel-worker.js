...
★최우선 식별 = "놓치면 사고나는 운영이슈"(AI가 트래킹·대체해야 할 대상):
  ...
  - 주문누락: 주문 접수/전달 과정에서 품목·수량이 빠짐·누락 (예: "우리것만 누락", "○○ 빠졌어요")
  - 출고누락: 출고/배차에서 빠짐·미출고
이런 운영이슈는 반드시 critical:true. 불량클레임·단순문의는 critical:false(자동화 우선순위 낮음).

분석 단위 = 이슈/케이스(같은 품목+차수로 묶는다). 마크다운·설명 없이 순수 JSON 하나만 출력:
{
 "cases":[{"key":"차수+품목 예 11-1 카네이션","product":"","seq":"","type":"주문|주문변경추가|주문변경취소|주문변경교체|항공스케줄|주문누락|출고누락|재고|불량클레임|정산|배차출고|문의|기타","critical":false,"raisedBy":"발신 직원명(로스터매칭)","customers":["내용에 언급된 거래처"],"resolved":true,"turns":1,"tone":"일상|급함|불만|정중|강경","evidence":"핵심 근거 원문 15자 이내 인용","summary":"한 줄"}],
