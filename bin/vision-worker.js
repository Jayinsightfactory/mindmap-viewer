...
다음 JSON 형식으로만 응답 (마크다운 없이 순수 JSON):
{
  ...
  "businessStage": "발주|입고|분배|현장출고|견적서|거래처전달|입금|해외송금|이익|기타 — 네노바 사업 흐름(영업 발주→입고→영업지원 분배→현장 출고→출고값 기준 견적서→거래처 전달→입금→해외농장 송금→영업이익) 중 이 화면이 속한 단계. 화면 근거가 없으면 '기타'",
  "purpose": "이 작업을 왜 하는가 1줄 — 어떤 업무 필요 때문인지(예: '37-1차 콜롬비아 장미 입고 물량을 거래처별로 나누기 위해'). 화면에서 읽히는 근거로만, 모르면 null",
  "outputArtifact": "이 작업이 끝나면 남는 결과물 1줄(예: '출고분배 저장 → 견적서 금액의 기준', '발주취합 엑셀 9월3주차 시트', '거래처 단톡방 회신'). 모르면 null",
