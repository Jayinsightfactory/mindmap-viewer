"use strict";function closeAllRightPanels(t){["messenger-panel","insight-panel","suggestion-panel","follow-panel","my-talent-panel","learning-data-panel"].forEach(s=>{if(s===t)return;const n=document.getElementById(s);n&&(n.style.display="none",n.classList.remove("open"))})}window.closeAllRightPanels=closeAllRightPanels;function escHtml(t){return String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function relTime(t){const e=Date.now()-new Date(t||0).getTime();return e<6e4?"방금":e<36e5?`${Math.floor(e/6e4)}분 전`:e<864e5?`${Math.floor(e/36e5)}시간 전`:`${Math.floor(e/864e5)}일 전`}function computeInsights(){const t=Date.now()-36e5,e=[..._allNodes].filter(o=>new Date(o.timestamp||o.created_at||0)>t).sort((o,d)=>new Date(d.timestamp||d.created_at||0)-new Date(o.timestamp||o.created_at||0)).slice(0,20),s={chat:{label:"💬 대화",color:"#58a6ff"},code:{label:"⚡ 코드",color:"#3fb950"},file:{label:"📄 파일",color:"#ffa657"},git:{label:"🌿 Git",color:"#39d2c0"},error:{label:"❌ 오류",color:"#f85149"}},n={};_allNodes.forEach(o=>{const d=typeCfg(o.type).cat||"chat";n[d]=(n[d]||0)+1});const l=Math.max(_allNodes.length,1),c=Object.entries(n).sort((o,d)=>d[1]-o[1]).map(([o,d])=>({...s[o]||{label:o,color:"#8b949e"},pct:Math.round(d/l*100)})),r={auth:"🔐 인증",api:"🌐 API",data:"🗄️ 데이터",ui:"🎨 UI",test:"🧪 테스트",server:"🚀 서버",infra:"🐳 인프라",fix:"🔧 버그수정",git:"🌿 Git",chat:"💬 대화",general:"⚙️ 일반"},i={};planetMeshes.forEach(o=>{const d=o.userData.domain||"general";i[d]=(i[d]||0)+1});const a=Object.entries(i).sort((o,d)=>d[1]-o[1])[0],p=a&&planetMeshes.length?`${r[a[0]]||a[0]} ${Math.round(a[1]/planetMeshes.length*100)}%`:"—",u=(_allNodes.filter(o=>o.type==="tool.error").length/l*100).toFixed(1),h=_allNodes.filter(o=>o.type==="git.commit"||o.type==="git.push").length,y={};_allNodes.forEach(o=>{const f=(o.data?.filePath||o.data?.fileName||"").replace(/\\/g,"/").split("/").pop();f&&f.includes(".")&&(y[f]=(y[f]||0)+1)});const g=Object.entries(y).sort((o,d)=>d[1]-o[1])[0]?.[0]||"—";return{recentEvents:e,distRows:c,topDomainLabel:p,errRate:u,gitCount:h,topFile:g,total:l}}const _INS_TYPE_ICON={"user.message":"💬","assistant.message":"🤖","assistant.response":"🤖","tool.start":"⚡","tool.end":"✅","tool.error":"❌","file.read":"📄","file.write":"✏️","git.commit":"🌿","git.push":"🚀"};let _insightTab="feed";function switchInsightTab(t){_insightTab=t,["feed","purpose","stats"].forEach(e=>{document.getElementById(`ins-tab-${e}`).classList.toggle("active",e===t);const s=document.getElementById(`ins-pane-${e}`);e===t?s.style.display="flex":s.style.display="none"}),t==="feed"?renderFeedTab():t==="purpose"?renderPurposeTab():t==="stats"&&renderStatsTab()}window.switchInsightTab=switchInsightTab;function renderFeedTab(){const t=computeInsights();document.getElementById("ins-feed-list").innerHTML=t.recentEvents.length?t.recentEvents.map(e=>`
       ...
       </div>`,document.getElementById("ins-feed-meta").textContent=`총 ${t.recentEvents.length}개 · 최근 1시간`}function buildPurposeCards(){const t={};return _allNodes.forEach(e=>{const s=e.purposeLabel||e.data?.purposeLabel;s&&(t[s]||(t[s]=[]),t[s].push(e))}),Object.entries(t).map(([e,s])=>{s.sort((g,o)=>new Date(g.timestamp||g.created_at||0)-new Date(o.timestamp||o.created_at||0));const n=s[0]?.timestamp||s[0]?.created_at,l=s[s.length-1]?.timestamp||s[s.length-1]?.created_at,c=s.find(g=>g.type==="user.message"),r=(c?.label||c?.data?.text||"").trim(),i=[],a=new Set;s.forEach(g=>{if(g.type!=="tool.start")return;const o=g.data?.toolName||g.label||"",d=g.data?.filePath||g.data?.fileName||"";let f=null;if(/Glob|Grep/.test(o)&&!/Edit|Write/.test(o))f={label:"Explore",icon:"🔍"};else if(/^Read$/i.test(o)&&!/Edit|Write/.test(o))f={label:"탐색",icon:"📂"};else if(/plan\.md/i.test(d)&&/Write|Edit/.test(o))f={label:"Plan",icon:"📋"};else if(/Write|Edit/.test(o))f={label:"구현",icon:"⚡"};else if(/Bash/i.test(o)){const b=g.data?.command||"";/git\s+commit/i.test(b)?f={label:"커밋",icon:"🌿"}:/git\s+push/i.test(b)?f={label:"푸시",icon:"🚀"}:f={label:"Bash",icon:"💻"}}f&&!a.has(f.label)&&(a.add(f.label),i.push(f))});const p={};s.forEach(g=>{const d=(g.data?.filePath||g.data?.fileName||"").replace(/\\/g,"/").split("/").pop();!d||!d.includes(".")||(p[d]||(p[d]={name:d,write:0}),["file.write","tool.start"].includes(g.type)&&/Write|Edit/.test(g.data?.toolName||"")&&p[d].write++)});const m=[...s].reverse().find(g=>g.type==="git.commit"),u=m?.data?.hash||m?.data?.commitHash||"",h=s.some(g=>g.type==="tool.error");let y;return h?y="❌":u?y="✅":i.length?y="⚡":y="⏳",{purposeLabel:e,trigger:r,agentSteps:i,files:Object.values(p),commitHash:u,hasError:h,statusIcon:y,firstTs:n,lastTs:l}}).sort((e,s)=>new Date(s.lastTs||0)-new Date(e.lastTs||0))}function renderPurposeTab(){const t=buildPurposeCards(),e=document.getElementById("ins-purpose-list");if(!t.length){e.innerHTML=`<div class="ins-purpose-empty">
    ...
    <div class="ins-kv-row">
      ...
      ${m}
         ...
         </div>`:"",p=t.gitHash?`<button onclick="ptRollback(${_ptActive},null)"
    ...
    <div class="sg-card-actions">
      ...
      </div>`;return}let p="";if(c.length){const m=c.filter(u=>u.severity==="high").length;p+=`<div style="font-size:10px;color:#f85149;font-weight:600;
