...
function toggleAssistant() {
  ...
  const isOpen = panel.classList.toggle('open');
  // 메신저 닫기 (겹침 방지)
  const msgPanel = document.getElementById('messenger-panel');
  if (msgPanel && isOpen) msgPanel.classList.remove('open');

  if (isOpen && !_astLoaded) {
    _astLoaded = true;
...
async function _loadAssistantContext() {
  if (!_orbitUser) return;
  try {
    const r = await fetch('/api/assistant/context', {
      headers: { 'Authorization': 'Bearer ' + _orbitUser.token }
    });
    if (!r.ok) return;
    const data = await r.json();
    const el = (id) => document.getElementById(id);
    if (el('ast-ctx-tasks')) el('ast-ctx-tasks').textContent = data.tasks_total || 0;
    if (el('ast-ctx-auto')) el('ast-ctx-auto').textContent = (data.automatable || []).length;
    if (el('ast-ctx-scripts')) el('ast-ctx-scripts').textContent = data.scripts_generated || 0;

    // 미확인 피드백 표시
    if (data.pending_feedback && data.pending_feedback.length > 0) {
      const bar = el('ast-feedback-bar');
      if (bar) {
        bar.style.display = 'block';
        el('ast-feedback-text').textContent =
          '새 피드백 ' + data.pending_feedback.length + '건: ' +
          data.pending_feedback[0].message.slice(0, 80);
      }
    }

    // 자동화 추천이 있으면 제안 버튼 업데이트
    if (data.automatable && data.automatable.length > 0) {
      const sugEl = document.getElementById('ast-suggestions');
      if (sugEl) {
        const extra = data.automatable.slice(0, 2).map(t =>
          `<button class="ast-suggest-btn" onclick="sendAssistantMsg('${t.name} 작업을 자동화해줘')">${t.name} 자동화 (${(t.score*100).toFixed(0)}%)</button>`
        ).join('');
        sugEl.innerHTML += extra;
      }
    }
  } catch {}
}

async function _loadAssistantHistory() {
  if (!_orbitUser) return;
  try {
    ...
    if (data.messages && data.messages.length > 0) {
      // 기존 환영 메시지 유지하되 이력 추가
      const area = document.getElementById('ast-messages');
      if (!area) return;
      // 환영 메시지 제거
      const welcome = area.querySelector('.ast-welcome');
      if (welcome) welcome.style.display = 'none';
      data.messages.forEach(m => _appendAssistantBubble(m.role, m.content));
      area.scrollTop = area.scrollHeight;
    }
  } catch {}
}

function _appendAssistantBubble(role, content) {
  const area = document.getElementById('ast-messages');
  if (!area) return;
  ...
  if (role === 'assistant') {
    // 마크다운 → HTML 변환 (코드 블록, 볼드, 리스트)
    let html = content
      .replace(/```(\w*)\n([\s\S]*?)```/g, (_, lang, code) => {
        const escaped = code.replace(/</g, '&lt;').replace(/>/g, '&gt;');
        return `<pre><code class="lang-${lang}">${escaped}</code></pre>` +
               `<button class="ast-copy-btn" onclick="navigator.clipboard.writeText(this.previousElementSibling.textContent)">코드 복사</button>`;
      })
      .replace(/\*\*(.*?)\*\*/g, '<b>$1</b>')
...
async function sendAssistantMsg(presetMsg) {
  ...
  if (!_orbitUser) {
    _appendAssistantBubble('assistant', '로그인이 필요합니다.');
    return;
  }

  // 환영 메시지 숨기기
  const welcome = document.querySelector('.ast-welcome');
  if (welcome) welcome.style.display = 'none';

  // 사용자 메시지 표시
  _appendAssistantBubble('user', message);

  // 로딩 표시
  const area = document.getElementById('ast-messages');
  const loadingDiv = document.createElement('div');
  loadingDiv.className = 'ast-loading';
  ...
  try {
    const r = await fetch('/api/assistant/chat', {
      ...
      headers: {
        ...
        'Authorization': 'Bearer ' + _orbitUser.token,
      },
    ...
    if (!r.ok) {
      _appendAssistantBubble('assistant', '오류가 발생했습니다. 다시 시도해주세요.');
      return;
    }

    const data = await r.json();
    _appendAssistantBubble('assistant', data.reply);

    // 스크립트 저장 알림
    if (data.savedScript) {
      const notice = document.createElement('div');
      notice.style.cssText = 'font-size:10px;color:#3fb950;padding:4px 14px;align-self:flex-start';
      notice.textContent = '스크립트 자동 저장됨: ' + data.savedScript.task_name;
    ...
    if (data.context) {
      const el = (id) => document.getElementById(id);
      if (el('ast-ctx-scripts')) el('ast-ctx-scripts').textContent = data.context.scriptsCount || 0;
    }

  } catch (e) {
    loadingDiv.remove();
    _appendAssistantBubble('assistant', '네트워크 오류: ' + e.message);
  }
}
  ...
  if (area) {
    ...
    area.innerHTML = `
      <div class="ast-welcome">
        <div class="ast-welcome-icon">AI</div>
        <div class="ast-welcome-text">
          대화가 초기화되었습니다.<br>
          무엇을 자동화하고 싶으신가요?
        </div>
        <div class="ast-suggestions">
          <button class="ast-suggest-btn" onclick="sendAssistantMsg('어떤 업무를 자동화할 수 있어?')">어떤 업무를 자동화할 수 있어?</button>
          <button class="ast-suggest-btn" onclick="sendAssistantMsg('Excel 반복 작업 자동화 스크립트 만들어줘')">Excel 자동화 스크립트</button>
        </div>
      </div>`;
  }
}
