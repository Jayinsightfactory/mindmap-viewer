...
const ORBIT_FEATURES = [
  ...
  { icon:'👥', name:'워크스페이스', color:'#58a6ff', desc:'회사 멤버 관리·초대·워크스페이스 설정',       badge:'회사',    action:()=>{ toggleFeatPanel(); openWorkspacePopup(); } },
  { icon:'💬', name:'팀 채팅',     color:'#3fb950', desc:'팀 채널 + 1:1 DM + @nenova AI 어시스턴트', badge:'채팅',    action:()=>{ toggleFeatPanel(); toggleMessenger(); } },
  ...
  { icon:'📖', name:'가이드',      color:'#79c0ff', desc:'3분 안에 시작하는 nenova 빠른 가이드',      badge:'튜토리얼', action:()=>{ toggleFeatPanel(); openGuidePopup(); } },
