...
## 14. 설치 환경진단 — "이 PC가 왜 매번 문제냐" (2026-06-18, 커밋 491e1ba)
특정 PC만 설치 실패/데이터 안 옴이 반복되면, **재설치(install-open)할 때 install.ps1이 환경진단을 자동 수집**한다(설치 끝 "[진단]" 블록). 수집: 백신제품·Defender실시간+orbit예외등록·**uiohook 키보드훅 로드결과**·화면캡처·자동시작등록(Task데몬/Task감시/HKCU/Startup)·node버전.
- **3곳에 남음**: ①설치 콘솔 즉시 출력 ②PC의 `~/.orbit/install-diag.log` ③서버 `install.diag` 이벤트.
- **원격 조회**: `GET /api/admin/install-diag?hostname=X` → 호스트별 최신 진단. (install.diag는 `/api/daemon/events`에도 뜸)
- **판독**: `uiohook=fail:...` + `defenderExcluded=false` 면 **백신/Defender가 키보드훅 차단**(예외등록 필요). `taskDaemon=false & hkcuRun=false` 면 **자동시작 등록 실패**(재부팅 후 안 뜨는 원인). `av`에 AhnLab/nProtect 뜨면 그 제품 예외등록.
- 즉 keyboard.chunk=0(§11)의 원인이 **차단인지 로드실패인지 설치 로그 한 방에 판별**된다.

## 15. 오래 죽었던 PC가 켜지면 CPU 폭주 — auto-doctor 백로그 폭탄 (2026-06-18)
**증상**: 장시간(특히 12h+) 꺼져있던 PC를 켜면 그 PC가 극심하게 렉/CPU 100%.
**원인**: 서버 `routes/auto-doctor.js`가 죽은 PC에 **safe-cmd(gitpull-worker/reclone-worker/capture-diag)를 30분마다 누적**해서 큐에 쌓는다. + 학습엔진 `capture-config`도 쌓임. 20h 죽으면 수십 개 누적 → **부활하는 순간 데몬이 한꺼번에 실행** → reclone(레포 재클론)·gitpull·capture가 동시 폭발 + Windows reclone은 EBUSY로 실패·재시도까지 → CPU 폭주 + 재시작 루프. (김빛나 NENOVA 실사고)
**즉시 조치**: `POST /api/admin/pg-commands-purge {hostname, actions:["reclone-worker","gitpull-worker","capture-diag","capture-config","config","update","restart"]}` 로 밀린 큐 비움(exec 등 내 명령은 actions에서 빼면 보존). 큐 확인: `GET /api/admin/pg-commands-inspect?hostname=`.
**근본 수정(커밋)**: auto-doctor에 ①`hasPendingCommands` 가드(미소비 명령 있으면 안 쌓음=백로그 상한) ②reclone-worker 자동 큐 제거(Windows에서 무용+폭탄). 재발 차단.
