...
if ($Token -and $Token.Length -gt 5) {
  ...
  for ($attempt = 1; $attempt -le 3; $attempt++) {
    try {
      ...
      # PC conflict check: 이미 다른 유저 config가 존재하면 경고 후 덮어쓰기 (재설치 차단 제거)
      if (Test-Path "$env:USERPROFILE\.orbit-config.json") {
        try {
          ...
          if ($existingUid -and $existingUid -ne "local" -and $existingUid -ne $uid) {
            Write-Host "  [WARN] Overwriting existing config (was: $existingUid, new: $uid)" -ForegroundColor Yellow
            "$(Get-Date -f 'yyyy-MM-dd HH:mm:ss') [WARN] PC conflict overwrite: $existingUid -> $uid" | Out-File $LOG_FILE -Append -ErrorAction SilentlyContinue
          }
        } catch {}
      }
