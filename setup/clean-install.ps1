...
$daemonContent = @"
# Auto-generated by clean-install. Do not edit.
`$ErrorActionPreference = 'Continue'
Set-Location '$DIR'
while (`$true) {
    try {
        node daemon\personal-agent.js 2>&1 | Out-File -FilePath '$OrbitDir\daemon.log' -Append -Encoding UTF8
    } catch {
        "`$(Get-Date -f 'yyyy-MM-dd HH:mm:ss') [CRASH] `$_" | Out-File -FilePath '$OrbitDir\daemon.log' -Append -Encoding UTF8
    }
