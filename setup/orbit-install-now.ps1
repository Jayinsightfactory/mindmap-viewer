...
try {
  ...
  if (-not $isAdmin -and $env:ORBIT_INSTALL_ELEVATED -ne '1') {
    Log 'Not admin - opening UAC. Click Yes.'
    $shell = if (Get-Command pwsh -ErrorAction SilentlyContinue) { (Get-Command pwsh).Source } else { 'powershell.exe' }
    $dl = "$REMOTE/api/install-now.ps1"
    $inner = "`$env:ORBIT_INSTALL_ELEVATED='1'; `$f=`"`$env:TEMP\orbit-install-now.ps1`"; Invoke-WebRequest -Uri '$dl' -OutFile `$f -UseBasicParsing; & `$f"
