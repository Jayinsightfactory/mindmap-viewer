...
REM -- Admin (wait after UAC so window does not flash-close) --
net session >nul 2>&1
if %errorLevel% neq 0 (
    echo.
    ...
    if !RC! neq 0 (
        echo   Install failed or cancelled. Log: %LOG%
        pause
    )
    exit /b !RC!
)
...
if not exist "%PS1_LOCAL%" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "try { [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; (New-Object Net.WebClient).DownloadFile('%PS1_URL%','%PS1_LOCAL%') } catch { exit 1 }" 2>>"%LOG%"
)
...
if !PS1_SIZE! LSS 500 (
    echo.
    echo   [ERROR] Downloaded file too small (!PS1_SIZE! bytes).
    echo   Log: %LOG%
    pause
    exit /b 1
)
