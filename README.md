...
이 저장소는 Orbit과 Nenova 관련 작업을 한곳에서 이어가기 위한 기준 저장소입니다.
여러 작업이 하나의 페이지에서 동시에 진행될 수 있으므로, 작업 단위는 페이지가 아니라 업무 흐름/패널/도메인으로 관리합니다.

현재 정리 기준은 [`WORKSPACE.md`](WORKSPACE.md)에 있습니다.
반복 수정 요청과 작업 기억은 [`WORK_MEMORY.md`](WORK_MEMORY.md)에 누적합니다.
...
## ⚡ 원클릭 시작 (어떤 PC에서든 복붙 한 줄)

퍼미션 설정 + 프로젝트 클론 + 훅 등록 + 서버 시작을 **한 줄로** 처리합니다.
