...
### 000. Karpathy식 에이전트 코딩 가드레일 (항상 적용)
GitHub 10만+ 스타 `CLAUDE.md` 계열의 핵심 원칙을 이 프로젝트에 맞게 적용한다.
이 규칙은 속도보다 정확성과 손상 방지를 우선한다.

#### 1) 구현 전에 먼저 생각한다
- 불확실하면 조용히 가정하지 말고, 가정/모호함/대안을 먼저 드러낸다.
- 사용자의 요구가 여러 의미로 해석되면 가장 위험이 낮은 해석을 밝히고 진행한다.
- 직원 PC, 데몬, 배포, 데이터 삭제/수정, 자동 실행처럼 위험한 작업은 특히 목적과 범위를 확인한다.

#### 2) 가장 단순한 작동 코드를 우선한다
- 요청 범위를 넘는 기능, 추상화, 설정 옵션을 만들지 않는다.
- 한 번만 쓰는 코드를 위해 새 프레임워크나 큰 구조를 만들지 않는다.
- 200줄로 만든 것이 50줄로 가능하면 줄인다.
- Nenova/Orbit 기존 패턴, 기존 라우트, 기존 데이터 구조를 먼저 사용한다.

#### 3) 수술식으로 변경한다
- 사용자가 요청한 문제와 직접 연결된 파일/라인만 수정한다.
- 주변 코드, 주석, 포맷, 구조를 “좋아 보이게” 바꾸지 않는다.
- 기존 죽은 코드나 낡은 구조를 발견하면 삭제하지 말고 보고한다.
- 내가 만든 변경으로 생긴 unused import/변수/함수만 정리한다.
- 모든 변경 라인은 사용자 요청과 추적 가능해야 한다.

#### 4) 검증 가능한 목표로 끝낸다
- “작동하게 해줘”를 “무엇을 실행해서 통과해야 하는지”로 바꿔서 처리한다.
- 버그 수정은 가능하면 재현 → 수정 → 검증 순서로 한다.
- 다단계 작업은 짧은 계획과 각 단계의 검증 방법을 함께 둔다.
- 검증하지 못한 부분은 완료처럼 말하지 않고, 남은 위험으로 명시한다.

이 규칙이 잘 적용된 결과는 diff가 작고, 불필요한 재작성과 추측 구현이 줄고,
질문은 실수 이후가 아니라 구현 전에 나온다.

### 00. 대화/수정요청 보존 프로토콜 (최우선)
**사용자의 대화 내용, 반복 수정 요청, 이미 해결한 오류는 삭제하거나 덮어쓰지 않는다.**

작업을 시작하기 전에 반드시 관련 단어를 먼저 검색한다:

```powershell
scripts/prework-search.ps1 "nenova" "관련검색어"
```
...
작업 후에는 `WORK_MEMORY.md`에 다음을 누적 기록한다:
- 사용자가 요청한 핵심 내용
- 검색한 관련 단어
- 기존 구현/커밋 확인 결과
- 실제 수정 파일
