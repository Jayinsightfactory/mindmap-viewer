...
// ─── OOM 방지: uncaughtException 핸들러 ─────────────────────────────────────
process.on('uncaughtException', (err) => {
  const isOOM = err.message?.includes('JavaScript heap out of memory')
             || err.code === 'ERR_WORKER_OUT_OF_MEMORY';
  if (isOOM) {
    console.error('[FATAL] OOM 감지 — 프로세스 종료:', err.message);
    process.exit(137);
  }
    ...
    try {
      ...
      ws.send(JSON.stringify({
        ...
        graph:      userId !== 'local' && userId !== 'anonymous'
                      ? await getFullGraphForUser(userId) : await getFullGraph(),
          ...
          try {
            ws.send(JSON.stringify({
              ...
              graph:    uid !== 'local' && uid !== 'anonymous'
                          ? await getFullGraphForUser(uid) : await getFullGraph(),
              sessions: await getSessionsForUser(uid),
              stats:    await getStats(),
            }));
          } catch (e) {
            console.error('[WS] channel.joined 오류:', e.message);
          }
        })();
      ...
      if (msg.type === 'annotation.create') {
        (async () => {
          try {
            ...
            const payload   = { type: 'event', event, graph: await getFullGraph() };

            if (channelId) broadcastToChannel(channelId, payload);
            else           broadcastAll(payload);
          } catch (e) {
            console.error('[WS] annotation.create 오류:', e.message);
          }
        })();
      }
        ...
        (async () => {
          try {
            ...
            const graph = uid !== 'local' && uid !== 'anonymous'
              ? await getFullGraphForUser(uid, msg.sessionId)
              : await getFullGraph(msg.sessionId);
            ws.send(JSON.stringify({ type: 'filtered', graph }));
          } catch (e) {
            console.error('[WS] filter 오류:', e.message);
          }
        })();
      }
      ...
      if (msg.type === 'auth') {
        (async () => {
          try {
            ...
            console.error('[WS] auth 오류:', e.message);
          }
        })();
      }
...
app.post('/api/hook', async (req, res) => {
  try {
    ...
    const fullGraph = await getFullGraph();                              // 충돌 감지용 전체 그래프
    ...
    // 충돌 감지
    const recentEventsForConflict = await getAllEvents(200);
    const newConflicts = [];
    for (const ev of events) {
    ...
    const _broadcastUserFiltered = async () => {
      for (const client of wss.clients) {
        if (client.readyState !== WebSocket.OPEN) continue;
        try {
          const uid = client._userId || 'local';
          const userGraph    = (uid !== 'local' && uid !== 'anonymous')
            ? await getFullGraphForUser(uid) : fullGraph;
          const userSessions = await getSessionsForUser(uid);
    ...
    if (channelClients.has(channelId)) broadcastToChannel(channelId, {
      type: 'update',
      graph: fullGraph, stats, sessions: await getSessions(), completedToolStarts,
      hookSource: { channelId, memberName },
      securityLeaks: leaks, conflicts: newConflicts, shadowAI: shadowFindings,
    });
    else await _broadcastUserFiltered();
      ...
      for (const client of wss.clients) {
        ...
        try {
          ...
          const g   = uid !== 'local' && uid !== 'anonymous'
                        ? await getFullGraphForUser(uid) : graph;
          const s   = await getSessionsForUser(uid);
...
setInterval(() => {
  ...
  _heapPressure = used.heapUsed > 400 * 1024 * 1024; // 400MB 이상이면 압력 상태
}, 10000);

setInterval(async () => {
  if (wss.clients.size === 0) return;
  if (_heapPressure) {
    console.warn('[ACTIVITY] 힙 압력 감지 — 활동 브로드캐스트 스킵');
    return;
  }
  try {
    // 유저별 중복 그래프 빌드 방지: uid별로 그룹핑
    const clientsByUid = new Map();
    for (const client of wss.clients) {
      if (client.readyState !== WebSocket.OPEN) continue;
      const uid = client._userId || 'local';
      if (!clientsByUid.has(uid)) clientsByUid.set(uid, []);
      clientsByUid.get(uid).push(client);
    }

    // 동시 그래프 빌드 제한: 최대 5명분만 처리
    let built = 0;
    for (const [uid, clients] of clientsByUid) {
      if (built >= 5) break;
      try {
        const graph = (uid !== 'local' && uid !== 'anonymous')
          ? await getFullGraphForUser(uid) : await getFullGraph();
        ...
        const payload = JSON.stringify({ type: 'activity', scores });
